...
| `adapters/service/mod.rs` | 327 | **El transporte de producción de `service`**: TLS crudo sobre el *loopback*, una conexión por petición (`Connection: close`, como el original). El reensamblado de fragmentos y las partes de la respuesta larga (`FragmentBuffer`, `split_response`) viven en `ServiceState`, compartido entre las conexiones de esta instancia de transporte —el equivalente por instancia a los campos estáticos del original—, no en la conexión ni en el proceso. `respond` traduce cada `FramedRequest` (`domain/protocol/framing.rs`): `echo=`/`cmd=` reentran por `adapters/channel/conversation.rs::answer`, que ya sabe de credencial; `fragment=`/`firm=`/`send=` la comprueban aparte con `credential_matches`, porque `answer` no la conoce. Pruebas en `adapters/service/tests.rs` (228), contra `respond` directamente, sin socket. |
