...
| `src/client.rs` | El cliente a prueba y qué cliente es: binario y un perfil aislado por almacén (`rsa`, `ec`, `token`), cada uno con su envoltorio y su raíz de confianza, y el perfil de lanzamiento. |
...
| `../testdata/site-driver/scripts/` | Los guiones, un módulo por familia: certificado, firma, ficheros, lote, servidor intermedio servido por HTTP, canal WebSocket y socket a mano. |
