...
# La primera vez que abre una tanda hacen falta ademas sus coordenadas: --os y --os-version se
# toman solos con `uname`; --subject-version, --transport y --store los da quien sondea
# (`just probe ... -- --subject-version <v> --transport <t> --store <a> run-pending`).
...
probe subject="" trust_root="" *args: autoscript build-ts
    ...
    cargo run --example probe -- --subject "$launcher" --trust-root "$trust_root" \
        "${dossier_args[@]}" "${coordinate_args[@]}" {{ args }} "${command_args[@]}"
