...
use crate::signing::domain::bridge::{
    DataRejection, PreviousSignature, SealedPreSignature, SignatureVerdict, XadesVariant,
    LIBRARY_FILE,
};
...
fn an_unsigned_pdf_reports_no_previous_signatures() {
    let report = parse_previous_signatures(r#"{"ok":true,"signatures":[]}"#).expect("es valida");
...
fn a_previous_signatures_answer_missing_the_list_is_a_malformed_answer() {
    assert!(parse_previous_signatures(r#"{"ok":true}"#).is_err());
}
