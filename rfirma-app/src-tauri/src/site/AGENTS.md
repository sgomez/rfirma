...
| `adapters/relay.rs` | 113 | **El transporte del servidor intermedio**: sin canal que sostener, la operación llega resuelta desde la propia invocación —`dat` inline, o descargada por `fileid` y descifrada—; el asa de respuesta sube la contestación y cierra el proceso, o avisa a la ventana si la subida se rechaza. Pruebas en `adapters/relay/tests.rs` (282), con el doble de servlets en memoria. |
...
| `domain/channel.rs` | 138 | El canal visto desde dentro: su cometido —con la credencial negociada, que puede ser ausente—, la ubicación donde escucha —sorteo, puerto fijo o servidor intermedio (`RelayChannelInfo`)—, sus situaciones (ADR-0009) y el asa del canal abierto, sin socket. `ChannelError::refused` lleva ya su propio `Refusal` clasificado, para cuando el fallo es el del servidor intermedio. Pruebas en `domain/channel/tests.rs` (31). |
...
| `domain/protocol/launch.rs` | 293 | La invocación de arranque: verbo, versión de protocolo, ubicación de canal (`ChannelLocation`, sorteada en la v4, fija en la v3, o `Relay` para un verbo de operación con `rtservlet`/`stservlet`) y credencial (`NegotiatedCredential`, ausente cuando la v3 o el servidor intermedio no la traen) (ID-245…ID-249). Aquí vive `THE_PORT_OF_THE_THIRD_PROTOCOL`, que `adapters/channel/bind.rs` reexporta, y `RelayChannelInfo`, la información de canal del servidor intermedio. Pruebas en `domain/protocol/launch/tests.rs` (333). |
