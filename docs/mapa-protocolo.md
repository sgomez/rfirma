...
| `keystore` | `UrlParameters`, `autoscript`: `batch`, `sign`, `cosign`, `countersign`, `selectcert`, `signandsave` | Gana al `ksb64` y se lee con su misma regla: el nombre visible o el de la constante, la familia NSS se obedece y el resto del catálogo de `AOKeyStore` sale con `SAF_08` | `getKeyStoreName` (`UrlParameters.java:382`) elige el almacén; los cuatro lanzadores lo construyen con él | **Desviación declarada** (ADR-0022). Más estricta: rFirma no abre más almacén que el suyo |
...
| `ver` | `UrlParametersForBatch`, `UrlParametersToLoad`, `UrlParametersToSave`, `UrlParametersToSelectCert`, `UrlParametersToSign`, `UrlParametersToSignAndSave` | Solo en el camino del servidor intermedio, donde es la versión de la operación: ausente vale `0`, un valor que no es entero vale `1`, y por encima de la versión 4 sale `SAF_21` antes de firmar. Por un canal ya abierto se ignora y manda `v` (ADR-0021) | Versión de protocolo de la operación (`0` por defecto); se lee solo si no llegó `v`, y si supera la versión 4 sale `SAF_21` (`ProtocolInvocationLauncher.java:301` y las cinco líneas hermanas) | **Igual** |
