...
fn what_breaks_after_the_consent_keeps_its_own_code() {
    for (failure, expected) in [
        (
            CycleFailure::Cycle(CycleError::Token(
                crate::identity::adapters::pkcs11::TokenError::new(
                    TokenSituation::TokenAbsent,
                    "no hay tarjeta",
                ),
            )),
...
fn a_broken_session_seal_is_not_a_signature_that_did_not_come_out() {
    let broken = CycleFailure::Cycle(CycleError::Seal(crate::signing::domain::SealMismatch));
