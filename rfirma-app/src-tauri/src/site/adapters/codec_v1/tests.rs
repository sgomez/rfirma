...
fn it_encodes_a_batch_exactly_like_the_fourth_protocol() {
    ...
    assert_eq!(V1Codec.encode(&outcome), V4Codec.encode(&outcome));
    assert_eq!(V1Codec.encode(&outcome), "PHhtbC8+|+/+/");
}

#[test]
fn it_encodes_the_cancellation_and_refusals_from_the_same_closed_catalogue() {
    assert_eq!(V1Codec.encode(&SiteOutcome::Cancelled), "CANCEL");
    let refused = V1Codec.encode(&SiteOutcome::Refused(SiteRefusal::ScratchUnwritable(
