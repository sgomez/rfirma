...
fn a_disk() -> InMemoryFiles {
    InMemoryFiles::new().with_folder(CONTRACTS)
}
...
fn a_pdf(files: &InMemoryFiles, name: &str, bytes: &[u8]) -> PathBuf {
    let path = PathBuf::from(CONTRACTS).join(name);
    files.put(&path, bytes);
    path
}

fn an_opened_pdf(files: &InMemoryFiles, name: &str, opened: &OpenedDocuments) -> (PathBuf, String) {
    let path = a_pdf(files, name, b"%PDF-1.7 de prueba");
...
fn the_tray_survives_being_read_again_with_its_names_badges_and_order() {
    ...
    let (_, first) = an_opened_pdf(&files, "contrato.pdf", &opened);
    let (_, second) = an_opened_pdf(&files, "nomina.pdf", &opened);

    record(&memory, &files, &opened, &first, None).expect("deberia anotarse");
    record(&memory, &files, &opened, &second, None).expect("deberia anotarse");

    let next_session = OpenedDocuments::new();
    let rows = listed_rows(&memory, &files, &next_session);
...
fn a_path_that_no_longer_answers_is_unavailable_and_revives_when_it_comes_back() {
    ...
    files.take_away(&path);
    let gone = listed_rows(&memory, &files, &opened);
    files.put(&path, b"%PDF-1.7 de vuelta");
    let back = listed_rows(&memory, &files, &opened);
...
fn availability_is_never_written_to_the_disk() {
    ...
    let written = std::fs::read_to_string(memory.state_file().path()).expect("deberia leerse");
    assert!(
...
fn only_forget_takes_a_row_out() {
    ...
    forget(&memory, &files, &opened, &first).expect("deberia olvidarse");

    let names: Vec<String> = listed_rows(&memory, &files, &opened)
        .into_iter()
        .map(|row| row.name)
        .collect();
...
fn a_row_opened_through_a_symlink_is_still_the_row_that_forget_takes_out() {
    let home = where_the_memory_lives();
    let linked = "/home/quien/enlace/contrato.pdf";
    let files = a_disk().with_link(linked, format!("{CONTRACTS}/contrato.pdf"));
    a_pdf(&files, "contrato.pdf", b"%PDF-1.7 de prueba");
    let memory = a_memory(home.path());
    let opened = OpenedDocuments::new();
    let id = opened.mint(Document::opened(linked));
    record(&memory, &files, &opened, &id, None).expect("deberia anotarse");

    forget(&memory, &files, &opened, &id).expect("deberia olvidarse");

    assert!(listed_rows(&memory, &files, &opened).is_empty());
}
...
fn a_document_that_was_open_before_gets_its_page_and_position_back() {
    ...
    let opened = OpenedDocuments::new();
    let (path, id) = an_opened_pdf(&files, "contrato.pdf", &opened);
    record(&memory, &files, &opened, &id, Some(a_placement(3))).expect("deberia anotarse");

    let again = opened.mint(Document::opened(path));
    let row = record(&memory, &files, &opened, &again, None).expect("deberia anotarse");
...
fn a_brand_new_document_does_not_inherit_the_position_of_another_one() {
    ...
    let opened = OpenedDocuments::new();
    let (_, first) = an_opened_pdf(&files, "contrato.pdf", &opened);
    record(&memory, &files, &opened, &first, Some(a_placement(3))).expect("deberia anotarse");

    let (_, second) = an_opened_pdf(&files, "nomina.pdf", &opened);
    let row = record(&memory, &files, &opened, &second, None).expect("deberia anotarse");
...
fn the_signed_document_is_the_only_row_that_gets_the_signed_badge() {
    ...
    let (_, id) = an_opened_pdf(&files, "contrato.pdf", &opened);
    record(&memory, &files, &opened, &id, None).expect("deberia anotarse");
    let landing = a_pdf(&files, "contrato_firmado.pdf", b"%PDF-1.7 firmado");

    note_signed(&memory, &files, &landing, &a_completed_cycle());
...
fn no_row_carries_the_path_the_backend_dedupes_by() {
    ...
    assert!(!told.contains(CONTRACTS));
}
...
fn a_listed_row_can_be_read_because_it_carries_a_usable_identifier() {
    ...
    let bytes = super::super::documents::bytes_of(&files, &next_session, &rows[0].id)
...
fn the_size_is_global_and_the_position_is_of_each_document() {
    ...
    let opened = OpenedDocuments::new();
    let (_, first) = an_opened_pdf(&files, "contrato.pdf", &opened);
    record(&memory, &files, &opened, &first, Some(a_placement(1))).expect("deberia anotarse");
...
fn a_document_gets_its_whole_page_set_back_and_not_just_a_page() {
    ...
    let memory = a_memory(home.path());
    let opened = OpenedDocuments::new();
    let (path, id) = an_opened_pdf(&files, "expediente.pdf", &opened);
    let placed = placed_on(PageSet::All);
    record(&memory, &files, &opened, &id, Some(placed.clone())).expect("deberia anotarse");
...
fn a_document_that_is_remembered_still_leaves_its_row_when_taken() {
    ...
    let row =
        take(&memory, &files, &opened, &id, Some(a_placement(3))).expect("deberia ponerse delante");

    assert_eq!(row.name, "contrato.pdf");
    assert_eq!(row.placement, Some(a_placement(3)));
    assert_eq!(listed_rows(&memory, &files, &opened).len(), 1);
}
...
fn a_document_passing_through_leaves_neither_row_nor_placement_when_taken() {
    ...
    let memory = a_memory(home.path());
    let opened = OpenedDocuments::new();
    let path = a_pdf(&files, "de-la-sede.pdf", b"%PDF-1.7\n");
...
fn remembrance_decides_the_row_and_belongs_to_the_document_not_to_the_file() {
    ...
    let memory = a_memory(home.path());
    let opened = OpenedDocuments::new();
    let path = a_pdf(&files, "contrato.pdf", b"%PDF-1.7\n");
    let passing = opened.mint(Document::passing_through(path.clone()));
    let remembered = opened.mint(Document::opened(path));

    take(&memory, &files, &opened, &passing, None).expect("deberia ponerse delante");
    assert!(listed_rows(&memory, &files, &opened).is_empty());

    take(&memory, &files, &opened, &remembered, None).expect("deberia ponerse delante");
    assert_eq!(listed_rows(&memory, &files, &opened).len(), 1);
}
...
fn the_tray_never_borrows_the_identifier_of_a_document_passing_through() {
    ...
    let memory = a_memory(home.path());
    let opened = OpenedDocuments::new();
    let (path, remembered) = an_opened_pdf(&files, "contrato.pdf", &opened);
    opened.mint(Document::passing_through(path.clone()));
    take(&memory, &files, &opened, &remembered, None).expect("deberia anotarse");
