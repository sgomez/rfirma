...
- El fichero más grande del backend es `ffi.rs`, con 993 líneas; detrás van
  ...
  (649), y detrás va `commands/mod.rs` (641); lo que los hace crecer es
...
| `commands/mod.rs` | 641 | **Las veintitrés órdenes de Tauri**, y nada más que sus cuerpos. |
| `commands/views.rs` | 423 | Los tipos que cruzan a la ventana y las conversiones que los producen (ID-80). |
...
| `commands/guards.rs` | 649 | Las cuatro guardas que ven todas las órdenes a la vez (ID-85), y las pruebas del descubrimiento de tipos. Solo en pruebas. |
...
| `app/version.rs` | 370 | Si hay una versión nueva publicada: el puerto de red doblable, la caché de 24 h y la comparación de versiones (ID-177, ID-178, ID-180, ID-182). |
...
| `releases.rs` | 89 | El único sitio que abre una conexión: le pregunta a GitHub por la última publicación y devuelve el cuerpo tal cual (ID-178, ID-182). |
...
| `memory/mod.rs` | 544 | El reparto de las siete memorias. Léelo antes que sus hermanos. |
| `memory/state.rs` | 501 | El estado que la aplicación acumula por su cuenta (ID-31), y lo **global** de la firma visible (ID-74). |
