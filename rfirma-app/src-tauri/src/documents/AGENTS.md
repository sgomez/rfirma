...
| `application/tests.rs` | El andamio de grada A del contexto: `InMemoryFiles`, un disco de mentira con carpetas, ficheros, atajos y los que se niegan a abrirse, y `SavingDialog`, un diálogo de guardar de mentira. Solo en pruebas. |
...
| `adapters/views.rs` | Destino, destino de una sola firma, PDF firmado, documento abierto, soltado y reciente, con su `From` desde `domain/told.rs` y `RecentRow`. Pruebas en `adapters/views/tests.rs`. |
