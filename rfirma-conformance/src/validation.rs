//! La validación de la suite contra una referencia: si el informe de un cliente conocido da lo que la
...
pub(crate) struct Known {
    pub id: String,
    #[serde(deserialize_with = "the_outcome_named")]
    pub outcome: Outcome,
    pub cause: String,
    pub note: String,
}

/// Lo que se sabe de antemano del informe de un cliente concreto; lo que no nombra, sale CONFORME.
...
pub(crate) fn validate(report: &Report, catalogue: &[Check], reference: &Reference) -> Validation {
    let discrepancies: Vec<Discrepancy> = catalogue
        ...
        .filter_map(|check| {
            ...
            let observed = report.state_of(&check.id).unwrap_or(CheckState::Pending);
            (observed != CheckState::Resolved(expected)).then(|| Discrepancy {
                id: check.id.clone(),
                expected: outcome_name(expected),
                observed: match observed {
                    CheckState::Pending => PENDING_NAME,
                    CheckState::Resolved(outcome) => outcome_name(outcome),
                },
...
"#;
    ...
    fn a_run(results: &[(&str, Outcome)], catalogue: &[Check]) -> (tempfile::TempDir, Report) {
        ...
        for (id, outcome) in results {
            report.resolve(id, *outcome, None, Duration::ZERO).unwrap();
        }
        (dir, report)
    }
    ...
    fn a_run_that_gives_what_the_reference_foresees_is_validated() {
        ...
        let (_dir, report) = a_run(
            &[
                ("a_greeting", Outcome::Compliant),
                ("a_save", Outcome::Noncompliant),
            ],
            &catalogue,
        );

        assert_eq!(
            validate(&report, &catalogue, &a_reference()),
            Validation::Validated
        );
    }
    ...
    fn a_result_other_than_the_foreseen_one_is_a_discrepancy_naming_both() {
        ...
        let (_dir, report) = a_run(
            &[
                ("a_greeting", Outcome::NotObservable),
                ("a_save", Outcome::Compliant),
            ],
            &catalogue,
        );
    ...
    fn a_pending_measurable_check_is_a_discrepancy_and_an_unmeasurable_one_never_counts() {
        let catalogue = the_catalogue_in(THREE_CHECKS).unwrap();
        let (_dir, report) = a_run(&[("a_greeting", Outcome::Compliant)], &catalogue);

        let Validation::Discrepant { discrepancies } =
            validate(&report, &catalogue, &a_reference())
        else {
            panic!("una comprobación medible pendiente no deja validar el informe");
        };
