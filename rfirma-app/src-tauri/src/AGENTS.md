...
- El fichero más grande del backend es `app/errand.rs`, con 2182 líneas; detrás
  ...
  (1230), y detrás va `commands/guards.rs` (714); lo que los hace crecer es
...
| `commands/mod.rs` | 1230 | **Las treinta y tres órdenes de Tauri**, y nada más que sus cuerpos. |
...
| `app/errand.rs` | 2182 | **El trámite de sede**: la operación que llega por el canal, el momento del consentimiento —que no se salta nunca— y lo que la sede recibe, más el trámite vivo del que sólo hay uno y el fichero de paso que se borra con él (ID-272, ID-275, ID-276, ID-280, ID-286). |
