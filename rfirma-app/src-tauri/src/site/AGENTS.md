...
| `adapters/servlets.rs` | 115 | `RelayServlets`, el servidor intermedio de producción sobre `reqwest::blocking`: compone la URL como `IntermediateServerUtil` (`op`, `v`, `id`, `dat`) y valida el servlet como el original —`http`/`https`, host no local, sin parámetros propios— (1.9.2, `UrlParameters.java:351-379`); TLS lo valida el almacén del sistema. Pruebas en `adapters/servlets/tests.rs` (38). |
