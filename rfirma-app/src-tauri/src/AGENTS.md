...
- El fichero más grande del backend es `app/errand.rs`, con 2146 líneas; detrás
  ...
  (1156), y detrás va `commands/guards.rs` (714); lo que los hace crecer es
...
| `commands/mod.rs` | 1156 | **Las treinta y tres órdenes de Tauri**, y nada más que sus cuerpos. |
| `commands/views.rs` | 671 | Los tipos que cruzan a la ventana y las conversiones que los producen (ID-80). |
...
| `commands/guards.rs` | 714 | Las cuatro guardas que ven todas las órdenes a la vez (ID-85), y las pruebas del descubrimiento de tipos. Solo en pruebas. |
...
| `app/frontier.rs` | 308 | **La frontera de errores**: el único sitio donde una situación del ID-29 se convierte en el código `SAF_NN` que recibe la sede (ID-288, ID-292). |
...
| `app/errand.rs` | 2146 | **El trámite de sede**: la operación que llega por el canal, el momento del consentimiento —que no se salta nunca— y lo que la sede recibe, más el trámite vivo del que sólo hay uno y el fichero de paso que se borra con él (ID-272, ID-275, ID-276, ID-280, ID-286). |
