...
/// autoridades y certificados de páginas web. **En una tarjeta el filtro no
/// se aplica** ([`signable_certificates`]): ahí no hay nada que esconder
/// porque todo lo que hay ya es firmable, y sin sesión iniciada una tarjeta
/// corriente —SoftHSM entre ellas, comprobado— no enseña ninguna
/// `CKO_PRIVATE_KEY`; filtrar igual la dejaría sin listar nada.
///
/// En un almacén NSS sí se aplica, y **sin vuelta atrás** (ID-190): antes, si
/// la ranura no enseñaba ninguna clave visible, se devolvía el almacén entero
/// sin filtrar, y esa vuelta atrás era la causa real de que un perfil de
/// Firefox con contraseña maestra enseñase sus ciento y pico entradas con las
/// CA dentro. Ahora, sin sesión iniciada, esa ranura se queda sin
/// certificados firmables. Antes de filtrar, cada ranura que exige sesión la
/// inicia a ciegas —sin PIN que ofrecer, ver [`log_in_before_listing`]—; el
/// intento solo tiene éxito en el almacén sin protección real que describe
/// ID-195 —un perfil NSS sin contraseña maestra, o el NSS propio de un `.p12`
/// instalado—, así que contra una contraseña maestra de verdad falla en
/// silencio, sin propagar el error: no hay PIN que pedir aquí, y `CKR_OK` de
/// una tarjeta compartiendo el turno con el mismo módulo tampoco es un fallo
/// de esta ranura. El perfil desechable de las pruebas se monta con
/// `--empty-password`, que es justo el caso en el que las claves ya se ven
/// sin necesitar sesión (`CKF_LOGIN_REQUIRED` a `false`), así que ninguna
/// prueba de grada B ejercita el intento a ciegas contra un token que de
/// verdad lo rechace —medido contra SoftHSM y contra un perfil NSS con
/// contraseña real en `docs/research/token-flags-login.md`—.
...
fn log_in_before_listing(
    ...
    match session.login(UserType::User, None) {
        ...
        Err(Error::Pkcs11(RvError::UserAlreadyLoggedIn, _)) => Ok(false),
        Err(_) => Ok(false),
    }
}
...
///
/// El filtro **no se aplica a una tarjeta** ([`StoreClass::Card`]): ahí no hay
/// nada que esconder porque todo lo que hay ya es firmable —una tarjeta o
/// SoftHSM no traen autoridades sueltas mezcladas con las claves de la
/// persona—, y sin sesión iniciada una tarjeta corriente (SoftHSM entre
...
/// [`list_certificates`] existe para evitar.
///
/// En un almacén NSS sí se aplica, sin vuelta atrás (ID-190): antes, si la
/// ranura no enseñaba ninguna clave visible, se devolvía el almacén entero
/// sin filtrar, y esa vuelta atrás era la causa real de que un perfil de
/// Firefox con contraseña maestra enseñase sus CA sueltas. Ahora, sin sesión
/// iniciada, la ranura NSS simplemente se queda sin certificados firmables.
fn signable_certificates(
    session: &Session,
    store: &Store,
    token_label: &str,
) -> Result<Vec<TokenCertificate>, TokenError> {
    if store.class() == StoreClass::Card {
