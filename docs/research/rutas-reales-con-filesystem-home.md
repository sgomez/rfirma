...
sandbox en `rfd`. Y las dos features son **excluyentes de forma dura**:
...
Comprobado en el sandbox: el `Gtk.FileChooserDialog` sí se hace visible en
...
| `listdir /tmp` | el `/tmp` privado del sandbox | el `/tmp` privado del sandbox |
...
existir dentro del sandbox** con `home` puesto; el `/tmp` que se ve es el
...
Idéntico a lo que midió el #22: la escritura contesta OK, el sandbox ve el
...
- **El ADR-0011 se queda, con el argumento arreglado.** La frase «conservar
  ...
  que rfirma dispone **no puede** devolver la ruta real dentro del sandbox, sea
