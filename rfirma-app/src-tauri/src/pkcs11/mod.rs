...
pub fn sign(reference: &CertificateRef, pin: &str, data: &[u8]) -> Result<Vec<u8>, TokenError> {
    // El estado de sesión iniciada de PKCS#11 es **del token dentro del
    // proceso**, no de la sesión: dos firmas a la vez se pisan, y la segunda
    // recibe `CKR_USER_ALREADY_LOGGED_IN` en vez de firmar. Se firma de una en
    // una, que es además lo único que el recorrido de firma pide.
