...
3. **Distribución de la librería nativa (ADR-0004):**
   ...
   * El sandbox —**del flatpak, no de rfirma**— cambia dos cosas que fuera son gratis: el **módulo PKCS#11 lo empaqueta el propio flatpak** (los del anfitrión no cargan dentro), y **toda entrada y salida de ficheros pasa por portales**, así que ahí la aplicación no conoce la ruta original de un documento. En el `.deb` y el `.rpm` las dos cosas se encienden solas. Ver `docs/research/flatpak-canal-unico.md`.
