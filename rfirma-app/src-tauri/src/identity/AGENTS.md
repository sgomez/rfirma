...
token—, que consume `site/adapters/nss.rs` (RD-08).
...
| `adapters/pkcs11/mod.rs` | 445 | La capa PKCS#11, y `RealToken`, el único adaptador de producción del puerto `Token`. |
...
| `application/certificates.rs` | 355 | Qué certificados hay, cuál eligió la ventana, cuál se recordó, qué estampa el recuadro, y instalar o quitar un `.p12` (ID-192, ID-197). Devuelve `TokenError` o `InstallError`, nunca una vista. Pruebas en `application/certificates/tests.rs` (247). |
...
| `ports.rs` | 68 | **Los dos puertos**: `Token` —listar, cómo pide el secreto, firmar e importar un `.p12`, con el listado de varios almacenes ya resuelto sobre el de uno— y `NssHost`. Los contextos de firma y sede lo reciben por argumento; que lo importen es deuda de #443. |
