...
| `headless` | No se lee | `CertFilterManager.isMandatoryCertificate:145`: con `true`, si un solo certificado pasa el filtro **no se enseña el diálogo** | **Hueco** (#615). Más estricta: rFirma siempre pregunta |
| `mandatoryCertSelection` | No se lee | `false` tiene el mismo efecto que `headless=true` | **Hueco** (#615) |
| `profile` | Se expande con el motor de políticas y cruza al puente | `ProtocolInvocationLauncherSign.java:153` y `…SignAndSave.java:150` lo **borran** antes de firmar | **Hueco** (#615). Más laxa: rFirma cruza al puente algo que el original nunca aplica |
...
| `filenameActualName` | No se lee | `ProtocolInvocationLauncherSignAndSave.java:337`: nombre propuesto en el diálogo de carga | **Hueco** (#615) |
