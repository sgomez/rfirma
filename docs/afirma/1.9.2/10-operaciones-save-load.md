...
| `SAF_05` | `ERROR_CANNOT_SAVE_DATA` | `save` | Excepción no cancelada del guardado en `ProtocolInvocationLauncherSave.java:102`. Un fallo al escribir los bytes no llega aquí: el diálogo avisa y vuelve a pedir destino (`JSEUIManager.java:808-824`). | No se ha podido guardar los datos (`ProtocolLauncher.5`). |
...
| `SAF_14` | `ERROR_OBSOLETE_APP` | `save` / `load` | Reservado para `ParameterNeedsUpdatedVersionException`, que nadie lanza: no se emite ([15](15-errores.md) §4.7). | La aplicación está obsoleta y no puede procesarse la petición (`ProtocolLauncher.14`). |
