//! El cliente a prueba resuelto según qué cliente es: el binario, el envoltorio que lo aísla, la
...
/// cliente y el módulo PKCS#11 del almacén.
...
pub(crate) fn resolve(
    ...
    let isolated = the_isolated_store_of(&binary, kind)?;
    let trust_root = trust_root
        .or_else(|| the_served_root(kind, isolated.served_root.clone(), Path::is_file))
...
fn the_isolated_store_of(binary: &Path, kind: ClientKind) -> Result<IsolatedStore, Vec<String>> {
