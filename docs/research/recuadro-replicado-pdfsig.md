...
Todo vive en `docs/research/assets/replicated-widget-driver/`, en esta misma rama:
...
Lo que arriba queda «sin medir» en las dos primeras viñetas ya está medido. El veredicto
del sondeo **no cambia**: sigue siendo *no replicar*.

### VALIDe

Los dos ficheros, `pages-all.pdf` y `pages-1-2.pdf`, subidos al validador oficial
...
1. **Un solo firmante.** VALIDe imprime **un** bloque `Firmantes`, no tres. Cuenta lo mismo
   que `pdfsig`: el recuadro replicado es una firma, no varias. Es la respuesta a la
   pregunta del ticket.
...
(**pdfium**) es distinto del `pdf.js` que ya se midió en el [#115](https://github.com/sgomez/rfirma/issues/115).

![pages-all.pdf en Chrome: el recuadro en las tres páginas](assets/replicated-widget-driver/chrome-pdfium-pages-all.png)

**pdfium pinta el recuadro en las tres páginas**, en la misma posición, como poppler. No es
el comportamiento estrictamente conforme que se temía —pintarlo solo en la página del `/P`,
dejando invisibles las otras dos estampaciones—: ningún visor de los probados lo hace.
...
De la lista de arriba siguen abiertos `qpdf --check` y `mutool clean -s` (no instalados),
Adobe Acrobat y Foxit, las páginas de tamaños o rotaciones distintas, y la cofirma sobre un
documento ya replicado. Ninguno de ellos cambiaría la recomendación: la limitación que la
decide es de diseño, no de tolerancia —al ser el mismo objeto, el `/Rect` es forzosamente
idéntico en todas las páginas—, y ningún validador la denuncia porque no hay nada que
denunciar sobre la firma.
