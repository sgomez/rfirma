...
> **Vocabulario retirado.** Las entradas anteriores a 2026-09 hablan de un
> `dispatcher` que puntuaba cada ticket en un *tier* (`standard`, `complex`,
> `oversized`) antes de construirlo. Ese worker ya no existe y no se elige
> tier por adelantado: un constructor que se queda sin contexto entrega su
> rama a otro, y un cambio que se le va de las manos sube la escalera de
> arreglos. Lo único que sobrevive de aquella puntuación es la llamada de
> «este ticket trae varios entregables», que hoy produce el `brief-author`
> como subproducto de su investigación y decide el knob `shape` de
> `developer-defaults.md`. Las entradas se conservan porque la señal —qué
> forma de ticket salió cara aquí y por qué— sigue valiendo; léelas
> traduciendo `oversized` por «varios entregables» y `complex`/`standard` por
> nada.

- #47 triaged `oversized` (4 bundled deliverables: app tree, justfile grid, quality tooling, two-lane CI + testdata kit) but, built at opus under `--build-multiple`, converged in ONE fix cycle to CLEAN. Mechanism: the scoring worker read breadth — four independent deliverables with no existing pattern to imitate, because `rfirma-app/` did not exist — not depth. Foundational scaffolding tickets in this repo are wide but shallow: each deliverable is boilerplate wiring with a well-specified target, and the fault lines it produced were themselves a sufficient order of work. Signal: a sub-issue whose fault lines are all "create <thing> from scratch per <ADR>" rather than "change interacting behaviour X" — score `complex` (opus), not `oversized`.
...
- El spec #46 completo (15 sub-issues) cerró con TODO a `complex`/opus salvo tres tickets que la triage puntuó `oversized` y se construyeron igualmente por el flag `--build-multiple`: #57 (4 superficies de UI), #59 (panel + PIN + progreso + clasificación de errores) y #62 (flatpak: manifiesto + vendorizado + garantía del .so + verificación en sandbox). Los tres salieron bien: 1, 1 y 0 ciclos de arreglo, ninguno volvió con media función, y la revisión confirmó las cuatro rebanadas en los tres casos. Señal para la calibración: en este repositorio un `oversized` de esta forma —varias superficies independientes pero cada una con su ficha de diseño ya escrita— es construible de una vez a opus. Lo que hizo que funcionara fue pasarle al constructor las líneas de fractura de la triage como ORDEN DE TRABAJO explícito, con instrucción de comitear cada rebanada al terminarla.
...
- #499 (standard/sonnet, 1 ciclo de arreglo) repite el mecanismo de #89: el ticket medía si el carril lento ejecuta las pruebas `#[ignore]`, y el builder dio por buena una aserción sin comprobar que ese carril las corriera de verdad, además de citar mal el ADR-0009; lo cazó la revisión. Segunda confirmación de la misma clase — el tier de triage no está mal, el sesgo de verificación en tickets de medición sí, y persiste pese al aviso de #89: al construir un ticket de medición, comprobar explícitamente que el guion demuestra lo que afirma antes de darlo por CLEAN.
- #82 (triaje: oversized → complex por veto propio): el brief-author de entonces detectó la directiva explícita de no dividir en el cuerpo del issue («van juntas porque separarlas deja el árbol rojo») y rebajó su propia puntuación a complex. Funcionó: 1 ciclo y fusionado. El veto de directiva está operando como se pretendía; no hace falta regla nueva.
...
- #169 y #177 se triaron `oversized` (model=none) pero se construyeron a opus por el default `shape: build` del repositorio, y los dos fusionaron tras un único ciclo de arreglo sin escalada. Dos de nueve, ambos primero-de-su-familia dentro del ticket: en este repositorio `oversized` dispara de más, y un ticket acotado a una sola costura debería puntuar `complex` aunque agrupe varias decisiones de ID.
- La triage de #177 informó de que sus tres bloqueantes (#172, #174, #175) «no estaban en main» cuando los tres se habían fusionado minutos antes en la misma tanda secuencial: se leyó un checkout obsoleto. Mecanismo: la triage dedujo de esa lectura obsoleta que «no hay patrón que imitar», y eso empujó también hacia `oversized`. Hay que hacer `fetch origin/main` antes de concluir que un patrón está ausente.
...
2026-09-06 spec=#340 sub=#353 model=opus effort=medium pr=#385 verdict=CLEAN cycles=0 mergefix=0 wave=— outcome=merged
2026-09-06 spec=#340 sub=#354 model=opus effort=medium pr=#386 verdict=CLEAN cycles=1 mergefix=0 wave=— outcome=merged
