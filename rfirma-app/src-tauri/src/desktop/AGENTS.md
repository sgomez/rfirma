...
| `adapters/tauri.rs` | 57 | Las cuatro órdenes del escritorio: invocación, versión publicada y manejadores de `afirma://`. |
| `adapters/views.rs` | 35 | Manejadores de `afirma://` y versión nueva. Sin pruebas propias. |
| `application/handlers.rs` | 55 | Quién atiende `afirma://`, del escritorio a Preferencias y de vuelta: lo que se puede saber, lo que se escribe y el nombre de catálogo de cada situación (ID-238…ID-240). Pruebas en `application/handlers/tests.rs` (66). |
| `application/invocation.rs` | 213 | La invocación desde fuera, `rfirma documento.pdf`: qué abre, qué hace la segunda y por dónde sale la URL `afirma://` que no es una ruta (ID-157…ID-160, ID-235, ID-236). Pruebas en `application/invocation/tests.rs` (265). |
