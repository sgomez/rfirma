...
| `domain/channel.rs` | 174 | El canal visto desde dentro: su cometido —con la credencial negociada, que puede ser ausente—, la ubicación donde escucha —sorteo, puerto fijo o servidor intermedio (`RelayChannelInfo`)—, sus situaciones (ADR-0009) y el asa del canal abierto, sin socket. `OpenChannel` puede traer un `Delivery`, la entrega ya resuelta de un transporte que no espera a un mensaje futuro (servidor intermedio): quien la retira decide cuándo dispararla. `ChannelError::refused` lleva ya su propio `Refusal` clasificado, para cuando el fallo es el del servidor intermedio. Pruebas en `domain/channel/tests.rs` (52). |
...
| `domain/protocol/launch.rs` | 301 | La invocación de arranque: verbo, versión de protocolo, ubicación de canal (`ChannelLocation`, sorteada en la v4, fija en la v3, o `Relay` para un verbo de operación con `stservlet`) y credencial (`NegotiatedCredential`, ausente cuando la v3 o el servidor intermedio no la traen) (ID-245…ID-249). `is_a_relay_launch` exige `stservlet` a propósito: la variante `fileid`-solo del cliente publicado (`fileid`+`rtservlet`+`key`, sin `stservlet` ni `id`) queda fuera de alcance, refusal SAF_03. Aquí vive `THE_PORT_OF_THE_THIRD_PROTOCOL`, que `adapters/channel/bind.rs` reexporta, y `RelayChannelInfo`, la información de canal del servidor intermedio. Pruebas en `domain/protocol/launch/tests.rs` (344). |
