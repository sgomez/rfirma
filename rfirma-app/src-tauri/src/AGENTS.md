...
- El fichero más grande del backend es `app/errand.rs`, con 1774 líneas; detrás
  ...
  (775), y detrás va `commands/guards.rs` (700); lo que los hace crecer es
...
| `lib.rs` | 404 | Registro de comandos, complementos y estados de Tauri, la instancia única (ID-160) y el arranque, que **obedece a `app/startup.rs` y no decide nada**: los tres puertos que le pasa, la ventana de sede y el canal sostenido (ID-324…ID-334). Empieza aquí para ver el cableado. |
...
| `commands/mod.rs` | 775 | **Las veintinueve órdenes de Tauri**, y nada más que sus cuerpos. |
...
| `commands/guards.rs` | 700 | Las cuatro guardas que ven todas las órdenes a la vez (ID-85), y las pruebas del descubrimiento de tipos. Solo en pruebas. |
...
| `app/errand.rs` | 1774 | **El trámite de sede**: la operación que llega por el canal, el momento del consentimiento —que no se salta nunca— y lo que la sede recibe, más el trámite vivo del que sólo hay uno y el fichero de paso que se borra con él (ID-272, ID-275, ID-276, ID-280, ID-286). |
