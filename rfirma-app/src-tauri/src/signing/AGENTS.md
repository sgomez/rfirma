...
| `application/session.rs` | 368 | La sesión **local**: prefirma, firma en el token y la postfirma que entrega el documento; la sesión a medias es la misma para los dos recorridos (ADR-0001). `CycleFailure` es todo lo que puede salir mal de abrir el documento a entregarlo. Pruebas en `application/session/tests.rs` (364). |
