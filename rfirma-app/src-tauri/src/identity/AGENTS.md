...
| `adapters/pkcs11/nss.rs` | 315 | Cómo entra un `.p12` en un almacén NSS propio: el descodificador de PKCS#12 de `libsmime3` por FFI, sin criptografía propia y dentro del turno del token (ID-192, ID-193, ID-194). Declara aquí el rasgo `NssHost` —la carga compartida de `libnss3.so` y el turno del token— y su `RealNssHost`, porque su único comprador es otro adaptador, `site/adapters/nss.rs`. Pruebas en `adapters/pkcs11/nss/tests.rs` (28). |
...
| `ports.rs` | 84 | **Los tres puertos**: `Token` —listar, cómo pide el secreto, firmar e importar un `.p12`, con el listado de varios almacenes ya resuelto sobre el de uno—, `InstalledFolder` —la carpeta de cada `.p12` instalado, con sus permisos— y `CertificateMemory`. Ningún otro contexto los importa. |
| `adapters/folder.rs` | 23 | `RealInstalledFolder`: crear, restringir al dueño y borrar la carpeta de un `.p12` instalado. |
