...
| `application/recents.rs` | 214 | La bandeja, del disco a la ventana: quién la lee, quién la escribe y el reparto del recuadro (ID-74, ID-75); `RecentRow` es la fila y `RecentsError` por qué no se anota; `note_signed` exige el `CompletedCycle` de la postfirma. Pruebas en `application/recents/tests.rs` (348). |
