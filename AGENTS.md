...
3. **Distribución de la librería nativa (ADR-0004):**
   ...
   * La guardia del ADR-0011 (`the_portal_path_never_crosses_to_the_window`, en `crossing/guards.rs`) construye cada tipo de salida desde su caso de uso con un enlace del portal y recorre el JSON resultante campo a campo. Descubre sola cada tipo declarado con `crossing!`, así que uno nuevo **no pasa desapercibido**, pero sí hay que decidir a mano dónde entra: o se construye en `crossings_from_a_portal_document`, o se declara en `OUTPUTS_WITH_NO_DOCUMENT_BEHIND` (solo si detrás no puede haber ningún documento). No hay lista de ficheros: las guardas descubren los `tauri*`, `views*` y `orders*` del `adapters/` de cualquier contexto; hay guardas hermanas que se ponen rojas si se olvida cualquiera de las dos cosas.
