...
* **Cadena de Tauri (`rfirma-app/src-tauri/`):** no compila en este equipo de desarrollo — faltan las bibliotecas de sistema (`libwebkit2gtk-4.1-dev`, `libsoup-3.0-dev`, `libjavascriptcoregtk-4.1` y compañía). El fallo aparece como `pkg-config exited with status code 1` dentro del `build.rs` de `javascriptcore-rs-sys` y no nombra el paquete que falta. La lista completa vive en el paso «Dependencias de sistema de Tauri» de `.github/workflows/ci.yml`; no se instalan aquí sin que alguien lo autorice, así que la cadena de Rust la verifica el CI.
