...
ciclo, `Bridge`, los dos motores que presta, `FilterEngine` y `PolicyEngine`
—su adaptador es `adapters/engines.rs`—, y el hilo que aloja el puente,
`IsolateHost`, cuyo adaptador es `adapters/isolate.rs` (#453). Los casos de uso
reciben el hilo como `impl IsolateHost` porque su método es genérico. La clave privada no
...
| `application/preview.rs` | 48 | La prefirma en seco: el ciclo entero con un `PK1` inventado, sin PIN y sin escribir, para pintar el sello de verdad (ID-136, ID-110). Pruebas en `application/preview/tests.rs` (95). |
| `application/session.rs` | 376 | La sesión **local**: prefirma, firma en el token y la postfirma que entrega el documento y anota el sello de firmado con el `CompletedCycle` en la mano; la sesión a medias es la misma para los dos recorridos (ADR-0001). `CycleFailure` es todo lo que puede salir mal de abrir el documento a entregarlo. Pruebas en `application/session/tests.rs` (279). |
...
| `ports.rs` | 38 | **Los cuatro puertos**: `Bridge`, `IsolateHost`, `FilterEngine` y `PolicyEngine`. Ninguno tiene entrada que firme (ADR-0001). |
