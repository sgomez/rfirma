...
| `adapters/channel/bind.rs` | 48 | Ata la ubicación del canal: uno de los puertos que sorteó la sede —siempre en `127.0.0.1` y **nunca el 63117 cuando la sede sorteó puertos**— o un puerto fijo, tal cual, aunque sea el 63117 (ID-215). Pruebas en `adapters/channel/bind/tests.rs` (100). |
...
| `application/errand/mod.rs` | 150 | Los verbos, y el reparto. **Léelo antes que sus hermanos**: es lo único que una orden llama. Pruebas en `application/errand/tests.rs` (1777). |
...
| `application/errand/tests.rs` | 1777 | Las pruebas del trámite entero, en grada A, con los vecinos doblados sobre el token y el hilo de `fixtures.rs`. Solo en pruebas; la guarda de dirección no lo lee. |
...
| `application/site.rs` | 116 | **La invocación de una sede**: **la negociación de arranque** —recibe el códec y el transporte de la raíz, en un solo sitio (RD-05)—, produce la ubicación del canal y abre el canal ahí, y decide si un rechazo sale por el socket o por la ventana (ID-214, ID-215, ID-248). Con un trámite vivo la segunda invocación se rechaza (ID-280). Pruebas en `application/site/tests.rs` (230). |
...
| `domain/protocol/launch.rs` | 150 | La invocación de arranque: puertos, versión de protocolo y credencial de canal, con `NegotiatedCredential` para la credencial exigida o ausente (ID-245…ID-249). Pruebas en `domain/protocol/launch/tests.rs` (159). |
