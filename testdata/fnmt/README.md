...
| --- | --- | --- | --- |
| `activo-rsa.p12` | `1234` | Camino feliz. RSA 2048, OCSP `good`. Es el que está en el token SoftHSM `rfirma-test`. | **2028-10-30 10:06:59 GMT** |
| `revocado-rsa.p12` | `1234` | Revocado de verdad, no caducado: la firma se construye pero la validación debe rechazarla. OCSP `revoked`, motivo `superseded`, desde 2024-10-30. | 2028-10-30 09:58:12 GMT |
| `caducado-rsa.p12` | `G5cp,fYC9gje` | Caducado: el rechazo debe ocurrir **antes** de pedir el PIN. | 2020-11-08 12:48:35 GMT |
...
> `PF_CADUCADO_EIDAS.p12` (de donde sale `caducado-rsa.p12`).
...
```
6e0cad97b78be2918ed54a64a0dd4f3f6e4c16e01b405ef0836fb91b77a3ffb4  activo-rsa.p12
a8ff78c1a7b13bcdc12347f683dd5395b6e0ac1d9c3cad23e3668823ae2b1425  revocado-rsa.p12
901df49ac10cceb0524c8cb50833d1407d0974f42f9d45a5b4b71c0eefa4e91f  caducado-rsa.p12
```
...
`activo-rsa.p12` **caduca el 2028-10-30**. La guardia va partida a propósito
(ADR-0014):

- la **prueba dura** vive en `rfirma-app/src-tauri/tests/kit_fnmt.rs`, es de
...
las fechas de este README y de `kit_fnmt.rs` se actualizan a la vez. La huella
