...
cuatro puertos del contexto: `ProtocolCodec`, `Transport`, `TrustStores` y
`LocalCaSlots`; los casos de uso no nombran ningún adaptador (#453).
...
| `adapters/frontier.rs` | 89 | **La única traducción de un rechazo del trámite** (`SiteRefusal`) a la vista de la ventana y al código `SAF_NN` de la sede, en un solo `match` (ADR-0009); y el código del canal y la cancelación. Pruebas en `adapters/frontier/tests.rs` (183). |
| `adapters/tauri.rs` | 104 | Las nueve órdenes del trámite de sede: desempaquetar el `State`, llamar a un verbo de `application/errand/` y traducir (RD-07). Ninguna decide ni guarda estado propio. |
...
| `application/errand/desk.rs` | 263 | La mesa del trámite (`ErrandDesk`, con el token y el hilo del puente como puertos) y los dos consentimientos que se deciden sobre ella: el orden de las cribas, la admisibilidad, la política y el recuadro (ID-258, ID-266, ID-272, ID-282). La de producción se arma aquí, no en la orden. |
| `application/errand/mod.rs` | 171 | Los verbos, y el reparto. **Léelo antes que sus hermanos**: es lo único que una orden llama. Pruebas en `application/errand/tests.rs` (1635). |
| `application/errand/outcome.rs` | 155 | El vocabulario de salida: `SiteOutcome` —lo que la sede recibe, sin versión ni código— y lo que queda para la ventana: `ErrandStep`, `SigningConsent`, `NoCertificate`, `Moment`. |
| `application/errand/replies.rs` | 108 | Las respuestas finales, y **el único sitio que escribe en el cable** (ID-322): identidad entregada, firma entregada, la que no salió y la cancelación. |
...
| `application/errand/tests.rs` | 1635 | Las pruebas del trámite entero, en grada A, con el token y el hilo del puente doblados desde `fixtures.rs`. Solo en pruebas; la guarda de dirección no lo lee. |
| `application/session.rs` | 133 | La sesión **de sede**: la prefirma que vuelve a pasar el filtro de la sede (ID-259), **la postfirma que no escribe nada** (ID-286, ID-264) y `SiteRefusal`, la situación de cada negativa sin traducir. Pruebas en `application/session/tests.rs` (116). |
| `application/site.rs` | 113 | **La invocación de una sede**: **la negociación de arranque** —recibe el códec y el transporte de la raíz, en un solo sitio (RD-05)—, abre el canal en uno de los puertos sorteados, y decide si un rechazo sale por el socket o por la ventana (ID-214, ID-215, ID-248). Con un trámite vivo la segunda invocación se rechaza (ID-280). Pruebas en `application/site/tests.rs` (207). |
...
| `application/startup/mod.rs` | 197 | Recibe la invocación y los puertos —códec, transporte, ranuras de la CA local, almacenes de confianza y abridor de ventana— y decide si se enseña la principal o se atiende un trámite de sede (ID-324, ID-328…ID-329, ID-334). Apunta **con qué momento se abre la ventana de sede**: el trámite o el callejón sin salida (ID-341). Pruebas en `application/startup/tests.rs` (370). |
...
| `ports.rs` | 103 | **Los cuatro puertos**: `ProtocolCodec`, `Transport`, `TrustStores` y `LocalCaSlots`, con el `ReplyHandle` por el que se contesta mucho después (RD-03, RD-04). Un cierre con la firma del transporte **es** un transporte. Pruebas en `ports/tests.rs` (33). |
